    ...
    public class AuthController : ControllerBase {
        ...
        public async Task<ActionResult> RegistrarProfessor(ProfessorViewModel registerProfessor) {
            ...
            if (result.Succeeded) {
                ...
                var completarTextoClaim = await _userManager.AddClaimAsync(user, new Claim("completartexto", "criar,editar,excluir,retornar,realizar"));
                if (!completarTextoClaim.Succeeded)
                {
                    throw new InvalidOperationException($"Erro ao associar claim" + $" ({completarTextoClaim.ToString()}) ao usuário '{user.Id}'.");
                }
                ...
                var contratoClaim = await _userManager.AddClaimAsync(user, new Claim("contrato", "criar,editar,excluir,retornar"));
                if (!contratoClaim.Succeeded)
                {
                    throw new InvalidOperationException($"Erro ao associar claim" + $" ({contratoClaim.ToString()}) ao usuário '{user.Id}'.");
                }

...
let idUpdate;
const baseURL = `https://localhost:44327/api/modulos`;
        ...
        <div class="col-12 col-md-6 my-2">
          <div class="position-relative">            
                ...
                <div class="botoesCard">
                  <button type="button" style=${ehAluno() ? 'display:none;' : ""} id="btn_editar" onclick="editar(${data[i].id})" data-toggle="modal"
                  ...
                  <button type="button" style=${ehAluno() ? 'display:none;' : ""} id="btn_excluir" onclick="deletaModulo(${data[i].id})"
                  ...
                  <div class="card-text">
                  <a href="listaAtividadesCompletaFrase.html" onclick="selecionarModulo(${data[i].id})">
                    <h1 class="linkModulo">${data[i].nomeModulo}</h1></a>
                  </div>
                </div>            
          </div>
        </div>`;
...
const saveProviderModulo = (data) => {
  const renamedData = {
    nomeModulo: data.descricaoModulo,
    imagem: imagemNomeModulo,
    imagemUpload: imageBase64Modulo,
  }
  const xhr = new XMLHttpRequest();

  xhr.open('POST', 'https://localhost:44327/api/modulos', true);
...
const formModulo = {
  ...
  validateFields() {
    const { descricaoModulo } = formModulo.getValue()
    if (descricaoModulo.trim() === "") {
      throw new Error("Por favor, preencha todos os campos")
    }
  },

  formatProvider() {
    let { descricaoModulo } = formModulo.getValue()
    return {
      descricaoModulo
    }
  },
...
const formModuloUpdate = {
  ...
  validateFields() {
    const { descricaoModuloUpdate } = formModuloUpdate.getValue()
    if (descricaoModuloUpdate.trim() === "") {
      throw new Error("Por favor, preencha todos os campos")
    }
  },

  formatProvider() {
    let { descricaoModuloUpdate } = formModuloUpdate.getValue()
    return {
      descricaoModuloUpdate
    }
  },
...
const saveProviderModuloUpdate = (data) => {
  const renamedDataUpdate = {
    id: idUpdate,
    nomeModulo: data.descricaoModuloUpdate,
    imagem: imagemNomeModuloUpdate,
    imagemUpload: imageBase64ModuloUpdate,
  }
  const xhrUpdate = new XMLHttpRequest();

  xhrUpdate.open('PUT', 'https://localhost:44327/api/modulos', true);
...
function selecionarModulo(event) {
  ...
  if (event) {
    localStorage.setItem('moduloCorrente', JSON.stringify({ event }));
  };
};
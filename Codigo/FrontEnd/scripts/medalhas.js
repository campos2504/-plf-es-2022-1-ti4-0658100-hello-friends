const baseURLIdModulo = `https://tishellofriends.azurewebsites.net/api/modulos`;
let baseURLMedalhas;
const medalhaURI = 'https://tishellofriends.azurewebsites.net/api/medalha';
let idUsuario;
let urlIdUsuario;
let ouro = 0;
let prata = 0;
...
function recuperaIdAluno() {
  ...
  urlIdUsuario = ''.concat('https://tishellofriends.azurewebsites.net/api/alunos', '/email/', idUsuario.user.email);
  ...
  fetch(baseURLIdModulo, {
    ...
    .then((data) => {
      ...
      for (i = 0; i < data.length; i++) {
        baseURLMedalhas = `https://tishellofriends.azurewebsites.net/api/modulos/medalha/${data[i].id}/${idAluno}`
        let request = new XMLHttpRequest();
...
function buscarMedalha(id) {
  let baseURLBuscaMedalhas = `https://tishellofriends.azurewebsites.net/api/medalha/${id}/${recuperaIdAluno()}`;
  let request = new XMLHttpRequest();
